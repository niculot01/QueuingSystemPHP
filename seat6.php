...
// Retrieve the customer's queue_time and status from the 'floor6' table
$query = $db->prepare("SELECT queue_time, status FROM floor6 WHERE queue_number = ?");
$query->execute([$_POST['queue_number']]);
$customer = $query->fetch();
...
// Check if the customer queue number exists in the 'floor6' table
if (!$customer) {
    echo "Error: Customer queue number not found in the 'floor6' table";
    exit();
}

// Update the customer's status to 'seated' in the 'floor6' table
$query = $db->prepare("UPDATE floor6 SET status = 'seated' WHERE queue_number = ?");
$query->execute([$_POST['queue_number']]);

// Redirect back to the queue page
header('Location: 6F_display.php');
exit();
?>

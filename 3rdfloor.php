...
$query = $db->prepare("SELECT * FROM floor3 ORDER BY status DESC, queue_time ASC");
$query->execute();
...
if (isset($_POST['id'])) {
    ...
    $query = $db->prepare("DELETE FROM floor3 WHERE id = :id");
...
$query = $db->prepare("SELECT * FROM floor3 WHERE status = 'serving' ORDER BY queue_time");
$query->execute();
$serving_customers = $query->fetchAll();

// Retrieve the queue data for customers who are waiting
$query = $db->prepare("SELECT * FROM floor3 WHERE status = 'waiting' ORDER BY queue_time");
$query->execute();
$waiting_customers = $query->fetchAll();

// Retrieve the queue data for customers who are seated
$query = $db->prepare("SELECT * FROM floor3 WHERE status = 'seated' ORDER BY queue_time");
$query->execute();
